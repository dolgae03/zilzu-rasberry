...
#define VCU_ID_1 0x180117EF
#define VCU_ID_2 0x180217EF
...
struct MCU_SEND_2{
    int8_t Controller_Temperture, Motor_Temperture, life_signal;
    bool running, mode;
    bool error[24];
...
// Vehicle Control Unit Sends 에서 보내는 값
struct can_frame *
VCU_SEND(uint16_t target_torque, uint16_t target_speed, int running, int mode, uint8_t life_signal, int socket_fd){ 
    ...
    *((uint16_t *)(&(frame -> data[2]))) = target_speed + (32000);
    ...
    return NULL;sudo ip link set can0 up type can bitrate 1000000
}
...
MCU_RECEIVE_2(struct can_frame * frame){
    ...
    data -> Controller_Temperture = -40 + (frame -> data[0]);
    data -> Motor_Temperture = -40 + (frame -> data[1sudo ip link set can0 up type can bitrate 1000000]);
    
    char c = frame ->data[2];

    printf("%d %d %d\n", data->Controller_Temperture, data->Motor_Temperture, c); //두 번째 줄에서 컨트롤러와 모터의 온도를 출력
